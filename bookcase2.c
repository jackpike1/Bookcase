...
/* Function to count number of bookcases that would be printed in the solution*/
int count_parents(Bookcase array[MAXIMUM], int solution);

/* Function to generate children until a happy bookcase is found*/
int find_solution(Bookcase cases[MAXIMUM]);
...
void test(void) {
    ...
    print_bookcase(cases[13]);

    /*solution = find_solution(cases);*/
    assert(count_parents(cases, solution) == 7);
...
int find_solution(Bookcase array[MAXIMUM]) {
    ...
    do {
        ...
        parent++;
        printf("Happy bookcase is in position %d\n", find_happy_bookcase(array));
